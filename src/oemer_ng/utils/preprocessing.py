...
def _non_max_suppression(boxes, iou_threshold):
    ...
    while len(boxes) > 0:
        ...
        boxes = [
            ...
            _calculate_iou(current['xyxy'], box['xyxy']) < iou_threshold
        ]
...
class ImagePreprocessor:
    ...
    def _postprocess_onnx_outputs(self, outputs, original_shape, preprocess_params, conf_threshold=0.25, iou_threshold=0.45):
    ...
    def _merge_overlapping_boxes(self, boxes, iou_threshold=0.3):
        ...
        box_list = sorted([list(b['xyxy']) + [b['cls'], b['conf']] for b in boxes], key=lambda x: x[5], reverse=True)
        ...
        for i in range(len(box_list)):
            ...
            if len(overlapping) > 1:
                ...
                merged_box = [min_coords[0], min_coords[1], max_coords[2], max_coords[3], current[4], max_conf]
    ...
    def _cleanup_with_onnx(
        ...
        boxes = self._postprocess_onnx_outputs(outputs, original_shape, preprocess_params, conf_threshold, iou_threshold)
